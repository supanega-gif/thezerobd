  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
      <header className="text-center">
        ...
        <h1 className="mt-3 font-display text-5xl md:text-6xl">From our kitchen, with care.</h1>
      ...
      {/* category nav */}
      <nav className="sticky top-16 z-30 mt-12 -mx-4 flex justify-center border-y border-border/60 bg-background/85 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6">
        <div className="flex flex-wrap gap-x-8 gap-y-2 text-sm">
          {menu.map((c) => (
            <a key={c.id} href={`#${c.id}`} className="text-muted-foreground transition-colors hover:text-primary">
              {c.title}
            </a>
          ))}
        </div>
